...
import prImg from '../../public/pp.jpg'
    ...
    return (
        <div className="flex">
            <div className="w-[28%] h-[100vh] bg-gray-900">
                ...
                <div className="h-[5vh] ml-4 flex mt-2">
                    <button onClick={clickChanger} >
                        <div className="absolute w-6 h-[20px] ml-[10px] flex -mt-3">
                            {
                                search.length > 0 || clicked != false
                                    ?
                                    (
                                        <svg viewBox="0 0 24 24" height="24" width="24" preserveAspectRatio="xMidYMid meet" version="1.1" x="0px" y="0px" enableBackground="new 0 0 24 24" xml:space="preserve"><path fill="currentColor" d="M12,4l1.4,1.4L7.8,11H20v2H7.8l5.6,5.6L12,20l-8-8L12,4z"></path></svg>
                                    ) :
                                    (
                    ...
                    <input className='w-[90%] h-full rounded-full pb-1 flex px-9 bg-slate-500' placeholder='Search or start new chat' onClick={clickChanger} value={search} onChange={handleChange} type="text" />
                    <button className='w-[10%] h-full rounded-full -mt-[1px] text-white flex justify-center items-center '>
                        <svg viewBox="0 0 24 24" height="20" width="20" preserveAspectRatio="xMidYMid meet" version="1.1" x="0px" y="0px" enableBackground="new 0 0 24 24" space="preserve"><path fill="currentColor" d="M10,18.1h4v-2h-4V18.1z M3,6.1v2h18v-2H3z M6,13.1h12v-2H6V13.1z" className='items-center'></path></svg>
                    </button>
                </div>
                <div className="h-[85.6vh] mt-[10px] ml-2 mr-2 mb-3 text-white overflow-scroll shadow-lg no-scrollbar cursor-pointer">
                    {
                        User.map((items, index) => {
                            return (
                                <React.Fragment key={index}>
                                    <div className="w-full h-[9vh] flex rounded-full mb-[2px]">
                                        <div className="w-[15%] justify-center items-center">
                                            <img src={items.image} className='ml-3 mt-[10px] w-[40px] rounded-full h-[40px]' alt={`Profile of ${items.name}`} />
                                        </div>
                                        <div className="w-[85%] mt-1">
                                            <div className="name mt-1">
                                                <h1><b>{`${items.firstName} ${items.lastName}`}</b></h1>
                                            </div>
                                            <div className="msg threedot mt-1">
                                               My message to you is, my eyeColor is {items.eyeColor}, and my address is {items.address.address}
                                            </div>
                                        </div>
                                    </div>
                                    <hr className='w-full' />
                    ...
                    <div className="w-full mt-1 mb-3 flex rounded-full">
                        <div className="mx-auto">
                            Made by <a href="https://github.com/ImBiswarup" target='_blank'><span className='pl-2 hover:underline'>Biswarup Ghosh</span></a>
                        </div>
                    </div>
                </div>
            </div>
